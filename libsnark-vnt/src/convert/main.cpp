                                                                   ...
                                                                   )
{
    ...
    convert_gadget<FieldT> convert(pb); // 构造新模型
    convert.generate_r1cs_constraints(); // 生成约束

    convert.generate_r1cs_witness(note_old, notes, note, cmtA_old, cmtS, cmtA); // 为新模型的参数生成证明
...
int main () {
    ...
    //value_old=value+value_s
    test_convert_gadget_with_instance<default_r1cs_ppzksnark_pp>(value_old, value_s, value);
